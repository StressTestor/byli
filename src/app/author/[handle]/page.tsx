 ...
 * Author Profile Page
 *
 * Server component. Fetches author by handle, lists their published articles
 * with engagement stats, categories, and read times.
 */
...
export async function generateMetadata({
  ...
  const { data } = await supabaseAdmin
    .from('authors')
    .select('display_name, handle, article_count')
    .eq('handle', handle)
    .single();
  ...
  const description = `Read ${author.article_count} article${author.article_count === 1 ? '' : 's'} by @${author.handle} on Linkdrift.`;
...
export default async function AuthorPage({
  ...
  // fetch published articles with stats and categories
        ...
        <div className="flex items-start gap-5 mb-10">
          {author.avatar_url ? (
            <img
              ...
              className="w-[128px] h-[128px] rounded-full border-2 border-zinc-800 flex-shrink-0"
            />
          ) : (
            <div className="w-[128px] h-[128px] rounded-full bg-zinc-800 flex items-center justify-center text-3xl font-bold text-zinc-500 flex-shrink-0 uppercase">
          ...
          <div className="min-w-0 pt-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              {author.verified && (
                <svg width="20" height="20" className="w-5 h-5 text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
              )}
            </div>

            <p className="text-sm text-zinc-500 mb-3">@{author.handle}</p>

            <div className="flex items-center gap-4 text-sm text-zinc-400">
              <span>{formatCount(author.follower_count)} followers</span>
              <span className="text-zinc-700">·</span>
              <span>{author.article_count} article{author.article_count === 1 ? '' : 's'}</span>
            </div>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 mt-4 text-sm text-zinc-400 hover:text-white transition-colors px-3 py-1.5 rounded-md border border-zinc-800 hover:border-zinc-600"
            >
              View on X
        ...
        <div className="border-t border-zinc-800/60 pt-6">
          ...
          {articleList.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-zinc-500">This author hasn&apos;t published any articles.</p>
            </div>
          ) : (
